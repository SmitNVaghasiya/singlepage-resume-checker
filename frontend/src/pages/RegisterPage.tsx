...
const RegisterPage: React.FC = () => {
    ...
    if (registerForm.username) {
      ...
      if (/\s/.test(username)) {
        ...
        setUsernameValidation((prev) => {
          if (
            prev &&
            (prev.message.includes("spaces") ||
              prev.message.includes("can only contain"))
          ) {
            return null;
          }
          return prev;
  ...
  const handleSendOtp = async () => {
    ...
    if (!registerForm.username.trim()) {
      ...
      const usernameValidation = UsernameValidator.validateBasic(
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (
      name === "username" &&
      usernameValidation &&
      !usernameValidation.message.includes("spaces") &&
      !usernameValidation.message.includes("can only contain")
    ) {
      setUsernameValidation(null);
    }
        ...
        <div className="auth-image-section">
          <div className="auth-image-overlay">
            <div className="auth-image-content">
              <h2 className="auth-image-title">Start Your Career Journey</h2>
              <p className="auth-image-subtitle">
                Join thousands of job seekers who've improved their resumes and
                landed better opportunities
              </p>
