...
class EmailService {
    ...
    async setupEtherealTransporter() {
        try {
            ...
            logger_1.logger.info('Using Ethereal test email service for development');
            logger_1.logger.info(`Test email credentials: ${testAccount.user} / ${testAccount.pass}`);
        }
        catch (error) {
            logger_1.logger.error('Failed to setup Ethereal email service:', error);
        }
    }
    async sendEmail(options) {
        try {
            ...
            if (process.env.NODE_ENV === 'development') {
                ...
                logger_1.logger.info(`Preview URL: ${nodemailer.getTestMessageUrl(info)}`);
            }
            else {
